...
/* ═══ HerAI CNN Architecture Builder — AI Lab port ═══ */
...
let AB_layerIdCounter = 0;
function AB_mkLayer(type, overrides = {}) {
  return { type, id: ++AB_layerIdCounter, ...JSON.parse(JSON.stringify(AB_LAYER_DEFAULTS[type] || {})), ...overrides };
}

const AB_PRESETS = {
  alexnet: [
    AB_mkLayer('conv',    { filters:96,  kernel:11, stride:4, padding:0 }),
    AB_mkLayer('relu'),
    AB_mkLayer('pool',    { type:'max', size:3, stride:2 }),
    AB_mkLayer('conv',    { filters:256, kernel:5,  stride:1, padding:2 }),
    AB_mkLayer('relu'),
    AB_mkLayer('pool',    { type:'max', size:3, stride:2 }),
    AB_mkLayer('conv',    { filters:384, kernel:3,  stride:1, padding:1 }),
    AB_mkLayer('relu'),
    AB_mkLayer('conv',    { filters:384, kernel:3,  stride:1, padding:1 }),
    AB_mkLayer('relu'),
    AB_mkLayer('conv',    { filters:256, kernel:3,  stride:1, padding:1 }),
    AB_mkLayer('relu'),
    AB_mkLayer('pool',    { type:'max', size:3, stride:2 }),
    AB_mkLayer('fc',      { units:4096 }),
    AB_mkLayer('relu'),
    AB_mkLayer('dropout', { rate:0.5 }),
    AB_mkLayer('fc',      { units:4096 }),
    AB_mkLayer('relu'),
    AB_mkLayer('dropout', { rate:0.5 }),
    AB_mkLayer('fc',      { units:1000 }),
  ],
  vgg16: [
    AB_mkLayer('conv',{ filters:64,  kernel:3, stride:1, padding:1 }), AB_mkLayer('relu'),
    AB_mkLayer('conv',{ filters:64,  kernel:3, stride:1, padding:1 }), AB_mkLayer('relu'),
    AB_mkLayer('pool',{ type:'max', size:2, stride:2 }),
    AB_mkLayer('conv',{ filters:128, kernel:3, stride:1, padding:1 }), AB_mkLayer('relu'),
    AB_mkLayer('conv',{ filters:128, kernel:3, stride:1, padding:1 }), AB_mkLayer('relu'),
    AB_mkLayer('pool',{ type:'max', size:2, stride:2 }),
    AB_mkLayer('conv',{ filters:256, kernel:3, stride:1, padding:1 }), AB_mkLayer('relu'),
    AB_mkLayer('conv',{ filters:256, kernel:3, stride:1, padding:1 }), AB_mkLayer('relu'),
    AB_mkLayer('conv',{ filters:256, kernel:3, stride:1, padding:1 }), AB_mkLayer('relu'),
    AB_mkLayer('pool',{ type:'max', size:2, stride:2 }),
    AB_mkLayer('conv',{ filters:512, kernel:3, stride:1, padding:1 }), AB_mkLayer('relu'),
    AB_mkLayer('conv',{ filters:512, kernel:3, stride:1, padding:1 }), AB_mkLayer('relu'),
    AB_mkLayer('conv',{ filters:512, kernel:3, stride:1, padding:1 }), AB_mkLayer('relu'),
    AB_mkLayer('pool',{ type:'max', size:2, stride:2 }),
    AB_mkLayer('fc',  { units:4096 }), AB_mkLayer('relu'), AB_mkLayer('dropout',{ rate:0.5 }),
    AB_mkLayer('fc',  { units:4096 }), AB_mkLayer('relu'), AB_mkLayer('dropout',{ rate:0.5 }),
    AB_mkLayer('fc',  { units:1000 }),
  ],
  resnet50: [
    AB_mkLayer('conv',   { filters:64, kernel:7, stride:2, padding:3 }), AB_mkLayer('bn'), AB_mkLayer('relu'),
    AB_mkLayer('pool',   { type:'max', size:3, stride:2 }),
    AB_mkLayer('residual',{ filters:64,  type:'bottleneck' }),
    AB_mkLayer('residual',{ filters:64,  type:'bottleneck' }),
    AB_mkLayer('residual',{ filters:64,  type:'bottleneck' }),
    AB_mkLayer('residual',{ filters:128, type:'bottleneck' }),
    AB_mkLayer('residual',{ filters:128, type:'bottleneck' }),
    AB_mkLayer('residual',{ filters:128, type:'bottleneck' }),
    AB_mkLayer('residual',{ filters:128, type:'bottleneck' }),
    AB_mkLayer('residual',{ filters:256, type:'bottleneck' }),
    AB_mkLayer('residual',{ filters:256, type:'bottleneck' }),
    AB_mkLayer('residual',{ filters:256, type:'bottleneck' }),
    AB_mkLayer('residual',{ filters:512, type:'bottleneck' }),
    AB_mkLayer('residual',{ filters:512, type:'bottleneck' }),
    AB_mkLayer('residual',{ filters:512, type:'bottleneck' }),
    AB_mkLayer('gap'),
    AB_mkLayer('fc', { units:1000 }),
  ...
  efficientb0: [
    AB_mkLayer('conv',   { filters:32, kernel:3, stride:2, padding:1 }), AB_mkLayer('bn'), AB_mkLayer('relu'),
    AB_mkLayer('conv',   { filters:16, kernel:3, stride:1, padding:1 }), AB_mkLayer('bn'), AB_mkLayer('relu'),
    AB_mkLayer('residual',{ filters:24,  type:'basic' }),
    AB_mkLayer('residual',{ filters:24,  type:'basic' }),
    AB_mkLayer('residual',{ filters:40,  type:'basic' }),
    AB_mkLayer('residual',{ filters:40,  type:'basic' }),
    AB_mkLayer('residual',{ filters:80,  type:'basic' }),
    AB_mkLayer('residual',{ filters:80,  type:'basic' }),
    AB_mkLayer('residual',{ filters:112, type:'basic' }),
    AB_mkLayer('residual',{ filters:192, type:'basic' }),
    AB_mkLayer('residual',{ filters:320, type:'basic' }),
    AB_mkLayer('conv',   { filters:1280, kernel:1, stride:1, padding:0 }), AB_mkLayer('bn'), AB_mkLayer('relu'),
    AB_mkLayer('gap'),
    AB_mkLayer('dropout',{ rate:0.2 }),
    AB_mkLayer('fc', { units:1000 }),
  ],
};
...
───────────────────────────────────────────────────────────── */
function initArchBuilderLayerEditor() {
  let layers = JSON.parse(JSON.stringify(AB_PRESETS.alexnet));
  ...
  // ── render layer list — NN-SVG style visual rows ──────────
  function renderList(shapes) {
    listEl.innerHTML = '';

    const maxSpatial  = Math.max(...shapes.map(s => s ? s.H : 1), 1);
    const maxChannels = Math.max(...shapes.map(s => s ? s.C : 1), 1);

    layers.forEach((l, idx) => {
      const sh     = shapes[idx + 1];
      const inSh   = shapes[idx];
      const color  = AB_LAYER_COLORS[l.type] || '#888';

      // ── visual dimensions ─────────────────────────────────
      // width  = channel depth (log)
      const vizW = sh
        ? Math.max(14, Math.min(120, Math.round(14 + (Math.log(sh.C + 1) / Math.log(maxChannels + 1)) * 106)))
        : 20;
      // height = spatial size (log), thin types get fixed slim height
      const SLIM = ['relu','bn','dropout'];
      const vizH = SLIM.includes(l.type) ? 6
        : sh ? Math.max(8, Math.min(64, Math.round(8 + (Math.log(sh.H + 1) / Math.log(maxSpatial + 1)) * 56)))
        : 16;
      ...
      vizBar.className = 'li-vizbar';

      // filter slice for conv (small square at left)
      if (l.type === 'conv' && l.kernel) {
        const fslice = document.createElement('div');
        fslice.className = 'li-filter-slice';
        const fh = Math.max(6, Math.round(vizH * 0.7));
        const fw = Math.max(4, Math.round(vizW * (l.kernel / 16)));
        fslice.style.cssText = `width:${fw}px;height:${fh}px;border-color:${color}cc;background:${color}50;`;
        vizBar.appendChild(fslice);
      }

      // main feature-map block
      const block = document.createElement('div');
      block.className = 'li-fm-block';
      block.style.cssText = `width:${vizW}px;height:${vizH}px;background:${color}22;border-color:${color}88;`;

      // grid lines inside block
      if (!SLIM.includes(l.type) && l.type !== 'gap' && vizH > 16) {
        const nLines = Math.min(5, Math.floor(vizH / 10));
        for (let i = 1; i < nLines; i++) {
          const ln = document.createElement('div');
          ln.style.cssText = `position:absolute;left:0;right:0;height:1px;top:${Math.round((vizH/nLines)*i)}px;background:${color}30;`;
          block.appendChild(ln);
        }
        block.style.position = 'relative';
      }
      vizBar.appendChild(block);
      ...
      summaryEl.className = 'li-summary';
      summaryEl.textContent = layerSummary(l);
      ...
      // controls: expand + delete
      const expandBtn = document.createElement('button');
      expandBtn.className = 'li-expand';
      expandBtn.textContent = '▸';
      expandBtn.title = 'Edit params';
      ...
      topRow.append(drag, badge, summaryEl, outEl, expandBtn, del);

      // params panel
      const panel = document.createElement('div');
      panel.className = 'li-panel';
      buildParamEditor(panel, l, () => { summaryEl.textContent = layerSummary(l); computeAll(); });

      content.append(topRow, panel);
      wrap.append(vizBar, content);

      // click to expand
      topRow.addEventListener('click', () => {
        const isOpen = wrap.classList.contains('open');
        listEl.querySelectorAll('.li-wrap').forEach(w => w.classList.remove('open'));
        if (!isOpen) {
  ...
  function layerSummary(l) {
    if (l.type === 'conv')     return `${l.filters}ch · ${l.kernel}×${l.kernel} · s${l.stride}`;
    if (l.type === 'pool')     return `${l.type_val || 'max'}pool ${l.size}×${l.size} s${l.stride}`;
    if (l.type === 'fc')       return `${l.units} units`;
    if (l.type === 'dropout')  return `p=${l.rate}`;
    if (l.type === 'residual') return `${l.filters}ch`;
    if (l.type === 'bn')       return 'Batch Norm';
    if (l.type === 'relu')     return 'ReLU';
    if (l.type === 'gap')      return 'Global Avg Pool';
    return '';
  }
  ...
  function buildParamEditor(el, l, onChange) {
    ...
    fields.forEach(([key, min, max, step, label]) => {
      ...
      row.className = 'li-param-row';
      const lbl = document.createElement('label');
      lbl.textContent = label;
      const inp = document.createElement('input');
      inp.type = 'range'; inp.min = min; inp.max = max; inp.step = step;
      inp.value = l[key] !== undefined ? l[key] : min;
      const valSpan = document.createElement('span');
      valSpan.className = 'li-param-val';
    ...
    if (l.type === 'pool') {
      ...
      ['max','avg'].forEach(v => {
        ...
        if ((l.pool_type || 'max') === v) o.selected = true;
        sel.appendChild(o);
      });
      sel.addEventListener('change', e => { e.stopPropagation(); l.pool_type = sel.value; onChange(); });
      sel.addEventListener('click', e => e.stopPropagation());
      row.append(lbl, sel);
      el.appendChild(row);
    }
  }
  // ── render pipeline — 2D flex visual ────────────────────────
  ...
  function renderPipeline(shapes) {
    ...
    function boxH(sh) {
      if (!sh) return 32;
      return Math.max(24, Math.min(160, Math.round(
        24 + (Math.log(sh.H + 1) / Math.log(maxSpatial + 1)) * 136
      )));
    }
    function boxW(sh, type) {
      if (type === 'relu' || type === 'bn' || type === 'dropout') return 10;
      if (type === 'gap') return 10;
      if (!sh) return 18;
      return Math.max(10, Math.min(40, Math.round(
        8 + (Math.log(sh.C + 1) / Math.log(maxChannels + 1)) * 32
      )));
    }
    ...
    scroll.appendChild(makePipeNode({
      label: 'IN', type: 'input', shape: inSh,
      color: '#6e6e73',
      h: boxH(inSh), w: boxW(inSh, 'input'),
    }));
    ...
    layers.forEach((l, i) => {
      const sh    = shapes[i + 1];
      const color = AB_LAYER_COLORS[l.type] || '#888';
      scroll.appendChild(makeArrow(color));
      scroll.appendChild(makePipeNode({
        label: layerAbbr(l), type: l.type, shape: sh,
        color, h: boxH(sh), w: boxW(sh, l.type),
        isResidual: l.type === 'residual',
        filterK: l.type === 'conv' ? (l.kernel || 3) : 0,
      }));
    });
  ...
  function makePipeNode({ label, type, shape, color, h, w, isResidual, filterK }) {
    const node = document.createElement('div');
    node.className = 'pv-node';
    node.title = shape ? `${shape.H}×${shape.W}×${shape.C}` : label;

    // filter slice for conv
    if (filterK > 0) {
      const fSlice = document.createElement('div');
      fSlice.className = 'pv-filter';
      const fh = Math.round(h * 0.55);
      const fw = Math.max(5, Math.round(w * filterK / 8));
      fSlice.style.cssText = [
        `height:${fh}px`, `width:${fw}px`,
        `border-color:${color}cc`,
        `background:${color}40`,
      ].join(';');
      node.appendChild(fSlice);
    }
    ...
    box.style.cssText = [
      ...
      `background:${color}18`,
      isResidual ? `border-style:dashed;border-color:${color}88` : '',
    ].filter(Boolean).join(';');

    // horizontal grid lines (feature map texture)
    const THIN_TYPES = ['relu','bn','dropout','gap','fc'];
    if (!THIN_TYPES.includes(type) && h > 40) {
      const lines = Math.min(6, Math.floor(h / 18));
      for (let i = 1; i < lines; i++) {
        const ln = document.createElement('div');
        ln.style.cssText = [
          'position:absolute', 'left:0', 'right:0', 'height:1px',
          `top:${Math.round((h / lines) * i)}px`,
          `background:${color}28`,
        ].join(';');
        box.appendChild(ln);
      }
    ...
    if (shape) {
      ...
      shEl.className = 'pv-shape';
      const txt = (type === 'fc' || type === 'gap')
        ? `${shape.C}`
        : `${shape.H}×${shape.C}`;
      shEl.textContent = txt;
      node.appendChild(shEl);
    }
  ...
  function makeArrow(color) {
    ...
    wrap.innerHTML = `<svg width="16" height="10" viewBox="0 0 16 10" fill="none">
      <line x1="0" y1="5" x2="12" y2="5" stroke="${color || 'rgba(255,255,255,.2)'}" stroke-width="1.2"/>
      <path d="M9 2l4 3-4 3" stroke="${color || 'rgba(255,255,255,.2)'}" stroke-width="1.2" fill="none" stroke-linecap="round" stroke-linejoin="round"/>
    </svg>`;
    return wrap;
  }
  // ── update stats bar ───────────────────────────────────────
  ...
  pBtns && pBtns.querySelectorAll('.preset-btn').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      if (btn.dataset.preset !== 'custom') {
        layers = JSON.parse(JSON.stringify(AB_PRESETS[btn.dataset.preset] || AB_PRESETS.alexnet));
      }
      computeAll();
    });
  });
  ...
  document.querySelectorAll('.le-add').forEach(btn => {
    btn.addEventListener('click', () => {
      layers.push(AB_mkLayer(btn.dataset.type));
      computeAll();
    });
  });
...
───────────────────────────────────────────────────────────── */
function initArchBuilderGradientFlow() {
...
───────────────────────────────────────────────────────────── */
function initArchBuilderEffNet() {
  const diagEl = document.getElementById('rbDiagram');
  ...
  function render() {
    const W = 520, H = blockType === 'basic' ? 280 : 360;
    const cx = W / 2;
    ...
    if (statsEl) {
      ...
      statsEl.innerHTML = `
        ...
        <span class="rbs-stat">Type: <strong>${blockType === 'basic' ? 'Basic (2×Conv 3×3)' : `Bottleneck (1×1→3×3→1×1, mid=${mid})`}</strong></span>
...
───────────────────────────────────────────────────────────── */
function initArchBuilderResBlock() {
  const slPhi = document.getElementById('slEffPhi');
  ...
  slPhi.addEventListener('input', () => {
    ...
    document.getElementById('valEffPhi').textContent = phi;
    buildGrid(SPECS[phi]);
  });
  buildGrid(SPECS[0]);
  buildScalingCompare();
...
───────────────────────────────────────────────────────────── */
function initArchBuilderUpload() {
  ...
  // ── File handling ──────────────────────────────────────────
  // Pick button — primary trigger
  if (udzBtn) {
    udzBtn.addEventListener('click', (e) => {
      e.preventDefault();
      e.stopPropagation();
      if (fileInput) { fileInput.value = ''; fileInput.click(); }
    });
  }
  // Drop zone click (excluding pick btn to avoid double-fire)
  if (dropZone) {
    dropZone.addEventListener('click', (e) => {
      if (e.target === udzBtn || udzBtn?.contains(e.target)) return;
...
function initArchBuilderMisc() {
  ...
  const labels = {
    ...
    'sec-gradient': 'Gradient Flow Sim',
    'sec-resblock': 'Residual Block Config',
    'sec-effscale': 'EfficientNet Explorer',
    'sec-upload':   'Upload & Feature Maps',
  };
  secs.forEach((sec, i) => {
...
/* ── Init ──────────────────────────────── */
window.initAiLabCnnArchBuilder = function() {
  var content = document.getElementById('arch-builder-content');
  if (!content || content.dataset.ready) return;
  content.dataset.ready = 'true';
  if (typeof initArchBuilderLayerEditor === 'function') initArchBuilderLayerEditor();
  if (typeof initArchBuilderGradientFlow === 'function') initArchBuilderGradientFlow();
  if (typeof initArchBuilderResBlock === 'function') initArchBuilderResBlock();
  if (typeof initArchBuilderEffNet === 'function') initArchBuilderEffNet();
  if (typeof initArchBuilderUpload === 'function') initArchBuilderUpload();
};
