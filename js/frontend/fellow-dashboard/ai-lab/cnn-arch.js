...
/* ═══ HerAI CNN Architecture — AI Lab port ═══ */
...
───────────────────────────────────────────────────────────── */
function initCnnArchPipelineBuilder() {
...
───────────────────────────────────────────────────────────── */
function initCnnArchGradientFlow() {
...
───────────────────────────────────────────────────────────── */
function initCnnArchCompoundScaling() {
...
───────────────────────────────────────────────────────────── */
function initCnnArchScalingCompare() {
...
───────────────────────────────────────────────────────────── */
function initCnnArchComparisonChart() {
...
/* ─────────────────────────────────────────────────────────────
   9. BACKBONE RECOMMENDER
───────────────────────────────────────────────────────────── */
function initCnnArchDecisionFramework() {
  ...
  const Qs = [
    { id:'deploy',   q:'Di mana model di-deploy?',
      opts:[{v:'mobile',l:'<i class="fas fa-mobile-screen"></i> Mobile / Edge'},{v:'server',l:'☁️ Server / Cloud'},{v:'offline',l:'<i class="fas fa-laptop-code"></i> Offline / Batch'}] },
    { id:'priority', q:'Prioritas utama?',
      opts:[{v:'speed',l:'⚡ Kecepatan'},{v:'accuracy',l:'<i class="fas fa-bullseye"></i> Akurasi'},{v:'balance',l:'⚖️ Seimbang'}] },
    { id:'task',     q:'Tipe task?',
      opts:[{v:'classify',l:'<i class="fas fa-tag"></i> Klasifikasi'},{v:'detect',l:'<i class="fas fa-magnifying-glass"></i> Detection'},{v:'segment',l:'<i class="fas fa-image"></i> Segmentasi'}] },
  ];
...
/* ── Init ──────────────────────────────── */
window.initAiLabCnnArch = function() {
  var content = document.getElementById('cnn-arch-content');
  if (!content || content.dataset.ready) return;
  content.dataset.ready = 'true';
  if (typeof initCnnArchPipelineBuilder === 'function') initCnnArchPipelineBuilder();
  if (typeof initCnnArchGradientFlow === 'function') initCnnArchGradientFlow();
  if (typeof initCnnArchCompoundScaling === 'function') initCnnArchCompoundScaling();
  if (typeof initCnnArchScalingCompare === 'function') initCnnArchScalingCompare();
  if (typeof initCnnArchBuilder === 'function') initCnnArchBuilder();
  if (typeof initCnnArchComparisonChart === 'function') initCnnArchComparisonChart();
  if (typeof initCnnArchDecisionFramework === 'function') initCnnArchDecisionFramework();
};
