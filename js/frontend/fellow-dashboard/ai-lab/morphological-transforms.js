...
/* ═══ HerAI Morphological Transforms — AI Lab port ═══ */

'use strict';

/* ── boilerplate ─────────────────────────────────────────────── */
function initMorphMisc0() {const b=document.getElementById('readBar');if(!b)return;function u(){const d=document.documentElement;b.style.width=Math.min(100,(window.scrollY/(d.scrollHeight-d.clientHeight))*100)+'%'}window.addEventListener('scroll',u,{passive:true});u();}
...
/* ═══════════════════════════════════════════════════════════════
   SE DESIGNER
═══════════════════════════════════════════════════════════════ */
function initMorphPresetEditor() {
...
═══════════════════════════════════════════════════════════════ */
let MORPH_pyodideInstance=null;
async function initPyodide(){
  try{
    const py=await loadPyodide();
    await py.loadPackage('numpy');
    MORPH_pyodideInstance=py;
...
async function runCode(code,outPre,checkEl,checkFn){
  ...
  try{
    let stdout='';
    MORPH_pyodideInstance.setStdout({batched:(s)=>{stdout+=s+'\n';}});
    await MORPH_pyodideInstance.runPythonAsync(code);
...
let MORPH_quizScore=0; const MORPH_quizAnswered={};
function initMorphQuiz() {
  const con=document.getElementById('quizContainer');
  if(!con)return;
  MORPH_quizData.forEach((q,qi)=>{
...
window.MORPH_answerQuiz=function(qi,oi){
  ...
  const correct=oi===q.correct;if(correct)MORPH_quizScore++;
...
function MORPH_checkFinalScore(){
  const totalQ=MORPH_quizData.length;
  const answered=Object.keys(MORPH_quizAnswered).length;
  ...
  const total=MORPH_quizScore+cs,max=totalQ+3;
  ...
  if(fsEl){
    ...
    document.getElementById('fsScores').innerHTML=
      '<div class="fs-score-item"><div class="fs-score-num">'+MORPH_quizScore+'/'+totalQ+'</div><div class="fs-score-lbl">Theory</div></div>'+
    ...
    document.getElementById('fsMsg').textContent=pct>=80?'<i class="fas fa-party-horn"></i> Excellent! Module 1 complete — you mastered morphological operations!':pct>=60?'<i class="fas fa-thumbs-up"></i> Good work! Review and try again.':'<i class="fas fa-dumbbell"></i> Keep practicing!';
  }
...
window.runPlayground=async function(){
  ...
  if(!MORPH_pyodideInstance){out.textContent='Python runtime not ready…';return;}
