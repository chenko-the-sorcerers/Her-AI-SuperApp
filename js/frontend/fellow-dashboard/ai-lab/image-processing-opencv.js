...
/* ═══ HerAI Image Processing with OpenCV — AI Lab port ═══ */
...
function initOpencvPixelInspector() {
...
/* ── 10. Pyodide ─────────────────────────────────────────────── */
let OPENCV_pyodideInstance = null;

async function initPyodide() {
  try {
    const py = await loadPyodide();
    await py.loadPackage('numpy');
    OPENCV_pyodideInstance = py;
...
async function runCode(code, outPre, checkEl, checkFn) {
  ...
  try {
    let stdout = '';
    OPENCV_pyodideInstance.setStdout({ batched: (s) => { stdout += s + '\n'; } });
    await OPENCV_pyodideInstance.runPythonAsync(code);
    if (outPre) {
...
function OPENCV_checkFinalScore() {
  ...
  if (fsEl) {
    ...
    fsMsg.textContent = pct >= 80
      ? '<i class="fas fa-party-horn"></i> Excellent! You\'ve mastered OpenCV basics. Ready for Filtering & Kernels!'
      : pct >= 60
        ? '<i class="fas fa-thumbs-up"></i> Good work! Review the incorrect answers before moving on.'
        : '<i class="fas fa-dumbbell"></i> Keep practicing! Re-read the sections and try again.';
    fsEl.style.display = 'block';
  }
...
window.resetCodingQuiz = function(num) {
  ...
  if (ed && OPENCV_cqDefaults[num]) ed.value = OPENCV_cqDefaults[num];
};

/* ── Playground ─────────────────────────────────────────────── */
const OPENCV_playgroundSnippets = {
...
const OPENCV_playgroundDefault = document.getElementById('playground-editor')?.value || '';

window.loadSnippet = function(name) {
  const ed = document.getElementById('playground-editor');
  if (ed && OPENCV_playgroundSnippets[name]) ed.value = OPENCV_playgroundSnippets[name];
};
...
window.runPlayground = async function() {
  ...
  if (!OPENCV_pyodideInstance) { out.textContent = 'Python runtime not ready yet…'; return; }
  try {
    let stdout = '';
    OPENCV_pyodideInstance.setStdout({ batched: (s) => { stdout += s + '\n'; } });
    await OPENCV_pyodideInstance.runPythonAsync(ed.value);
...
window.resetPlayground = function() {
  const ed = document.getElementById('playground-editor');
  if (ed) ed.value = OPENCV_playgroundDefault;
...
/* ── Init ──────────────────────────────── */
window.initAiLabOpencv = function() {
  ...
  if (typeof initOpencvFlipRotate === 'function') initOpencvFlipRotate();
  if (typeof initOpencvBitwise === 'function') initOpencvBitwise();
  if (typeof initOpencvQuiz === 'function') initOpencvQuiz();
