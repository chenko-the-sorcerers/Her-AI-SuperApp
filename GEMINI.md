...
5. **Content Pagination:** Jangan gabungkan materi yang sangat panjang ke dalam 1 halaman statis yang memanjang ke bawah (akan membosankan dan bikin malas baca). Selalu pecah materi menjadi beberapa sub-modul kecil menggunakan struktur file `chapters/` dan muat secara dinamis via JavaScript (seperti Modul 2 Python).
6. **Dynamic Sidebar Lists:** Saat menduplikasi layout (contoh: dari Modul 1 statis ke Modul 2 & 3 dinamis), pastikan daftar materi di `sidebar` (`<ol id="...">`) merepresentasikan chapter internal dari modul tersebut (dinamis menggunakan `window.loadChapter()`), BUKAN merepresentasikan modul utama. Gunakan JS untuk mengontrol `class="active"`.
